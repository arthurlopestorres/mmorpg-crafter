        ...
        if (div) {
            div.innerHTML = listaMaterias.map(m => {
                ...
                <div class="item" style="background-color: ${color}; color: white;" data-componente="${m.nome}">
                    ...
                    ${btnFabricarHtml}
                    <button class="btn-atualizar-estoque" data-componente="${m.nome}">Atualizar Estoque</button>
                    <div class="atualizar-estoque-input" id="update-${id}" style="display:none;">
                        <input type="number" class="input-nova-qtd" data-componente="${m.nome}" value="${formatQuantity(m.disp)}" min="0">
            ...
            // Novo: Adicionar event listeners para botões atualizar estoque
            document.querySelectorAll("#listaFarmar .btn-atualizar-estoque").forEach(btn => {
                btn.addEventListener("click", (e) => {
                    const item = e.target.closest(".item");
                    const componente = item.dataset.componente;
                    const inputDiv = document.getElementById(`update-farmar-${componente.replace(/\s/g, '-')}`);
                    inputDiv.style.display = "flex";
                    const input = inputDiv.querySelector(".input-nova-qtd");
                    input.focus();
                    input.select();
                });
            });
            // Adicionar event listeners para botões salvar e cancelar
            document.querySelectorAll("#listaFarmar .btn-save-stock").forEach(btn => {
                btn.addEventListener("click", async () => {
                    const inputDiv = btn.closest(".atualizar-estoque-input");
                    const input = inputDiv.querySelector(".input-nova-qtd");
                    const componente = input.dataset.componente;
                    ...
                    await atualizarEstoqueComponente(componente, novaQtd);
                });
            });
            document.querySelectorAll("#listaFarmar .btn-cancel-stock").forEach(btn => {
                btn.addEventListener("click", () => {
                    const inputDiv = btn.closest(".atualizar-estoque-input");
                    inputDiv.style.display = "none";
                });
            });
            // Adicionar event listener para enter no input (salvar)
            document.querySelectorAll("#listaFarmar .input-nova-qtd").forEach(input => {
                input.addEventListener("keydown", async (e) => {
                    if (e.key === "Enter") {
                        const inputDiv = e.target.closest(".atualizar-estoque-input");
...
// Novo: Função para atualizar estoque de componente específico
async function atualizarEstoqueComponente(componente, novaQtd) {
    const currentGame = localStorage.getItem("currentGame") || "Pax Dei";
    try {
        const data = await safeApi(`/estoque/set?game=${encodeURIComponent(currentGame)}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ componente, novaQuantidade: novaQtd })
        });
        if (data.sucesso) {
            // A atualização via websocket cuidará da UI
        } else {
